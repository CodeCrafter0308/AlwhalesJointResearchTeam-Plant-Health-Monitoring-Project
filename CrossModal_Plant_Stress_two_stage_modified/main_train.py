...
def train_one_fold(fold_id, train_recs, val_recs, out_dir, device, g_batch, fp_batch, voc_mask_true):
    ...
    # 类别平衡采样器，防止 Batch 内 period 标签不平衡导致训练发散
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        opt, max_lr=MAX_LR, steps_per_epoch=len(dl_tr), epochs=EPOCHS, pct_start=0.1
    )
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        for x, y_s, y_p in dl_tr:
            ...
            # 关键修改：训练阶段对 period 使用 teacher forcing，让模型严格遵循“先 stress 再 period”
            out = model(x, g_batch, fp_batch, y_stress=y_s)

            loss = model.loss(
                out, y_s, y_p, voc_mask_true,
                w_stress=1.0,
                w_period_cls=1.0,
                w_period_reg=1.5,   # 连续性约束，压低 period 的抖动
                w_guidance=2.0,
                label_smoothing=0.1,
            )
        ...
        with torch.no_grad():
            for x, y_s, y_p in dl_va:
                ...
                # 验证阶段不喂真值，让 period 只能使用 stress 的预测作为条件，模拟真实推理路径
        ...
        print(
            f"[Fold {fold_id}] Epoch {ep:03d} | LR: {current_lr:.2e} | "
...
def plot_cv5_curves(all_histories, out_dir):
    ...
    plt.tight_layout()
    save_path = out_dir / "cv5_accuracy_curves_two_stage.png"
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"\n✅ 两阶段模型的 5 折精度曲线图已保存至: {save_path}")
